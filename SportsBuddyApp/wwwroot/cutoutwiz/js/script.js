...
$document.ready(function () {
  ...
  function toggleSwiperInnerVideos(swiper) {
    ...
    setTimeout(function(){
      if (nextSlide.find('.rd-video').hasClass('rd-video-stop-autoplay')) {
        console.log('qew');
        return false;
      }
  ...
  if (plugins.rdVideoPlayer.length) {
    ...
    plugins.rdVideoPlayer.each(function(index, item){
      ...
      $window.on("scroll", $.proxy(function () {
        ...
        if (!firstPlayDelay || video.hasClass('rd-video-stop-autoplay') ||  video.hasClass('state-pause')) {
          return true;
        }

        if (isScrolledIntoView(video) && video.find("video")[0].paused) {
          video.find("video")[0].play();
          video.addClass("played");
        } else if(!isScrolledIntoView(video) && !video.find("video")[0].paused){
  ...
  if (plugins.pageLoader.length > 0) {
    $window.on("load", function () {
      plugins.pageLoader.addClass("load");

      setTimeout(function () {
        plugins.pageLoader.addClass("hide");
      }, 4800);
    });
  }
